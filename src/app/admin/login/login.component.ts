...
@Component({
    selector: 'app-login',
    standalone: true,
    imports: [ButtonModule, CheckboxModule, InputTextModule, PasswordModule, FormsModule, ReactiveFormsModule, FormsModule, RouterModule, RippleModule],
...
export class LoginComponent {
    ...
    login(): void {
        ...
        let value = LoginUserData.filter((user) => user.companyUrl === this.adminLogin.get('companyUrl')?.value && user.username === this.adminLogin.get('username')?.value && user.password === this.adminLogin.get('password')?.value);
