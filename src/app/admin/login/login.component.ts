...
@Component({
    selector: 'app-login',
    standalone: true,
    imports: [ButtonModule, CheckboxModule, ToastModule, InputTextModule, PasswordModule, FormsModule, ReactiveFormsModule, RouterModule, RippleModule, DropdownModule],
...
export class LoginComponent {
    ...
    handleAdminLogin(username: string, companyName: string, password: string): boolean {
        ...
        if (username === 'admin' && password === 'admin' && companyName === 'HiDoctor') {
        }
    ...
    handleEmployeeLogin(username: string, companyName: string, password: string): boolean {
        ...
        const matchedEmployee = employees.find((emp: any) => emp.email === username && emp.password === password);
        ...
        this.sessionStorage.setSite(matchedEmployee.siteId);
